...
createTestcaseInStory: function (rallyConf, projectID, workProductID, testcaseName) {
  request = rally({
    user:       rallyConf.RALLY_USERNAME,
    pass:       rallyConf.RALLY_PASSWORD,
    apiKey:     rallyConf.RALLY_API_KEY,
    apiVersion: 'v2.0',
    server: 'https://rally1.rallydev.com',
    requestOptions: rallyConf.REQUEST_OPTIONS,
  });
...
createDefectInStory: function (rallyConf, projectID, workProductID, defectName) {
  request = rally({
    user:       rallyConf.RALLY_USERNAME,
    pass:       rallyConf.RALLY_PASSWORD,
    apiKey:     rallyConf.RALLY_API_KEY,
...
createTestCaseResult: function (rallyConf, testcaseID, build, verdict, notes){
  request = rally({
    user:       rallyConf.RALLY_USERNAME,
    pass:       rallyConf.RALLY_PASSWORD,
    apiKey:     rallyConf.RALLY_API_KEY,
...
updateTestCaseLastResult: function (rallyConf, testcaseID, build, verdict, notes){
  request = rally({
    user:       rallyConf.RALLY_USERNAME,
    pass:       rallyConf.RALLY_PASSWORD,
    apiKey:     rallyConf.RALLY_API_KEY,
...
updateDefectResult: function (rallyConf, defectID, name, state, description){
  request = rally({
    user:       rallyConf.RALLY_USERNAME,
    pass:       rallyConf.RALLY_PASSWORD,
    apiKey:     rallyConf.RALLY_API_KEY,
...
addDefectToTestcase: function (rallyConf, testcaseID, defectName){
  request = rally({
    user:       rallyConf.RALLY_USERNAME,
    pass:       rallyConf.RALLY_PASSWORD,
    apiKey:     rallyConf.RALLY_API_KEY,
...
//workspaceID  optional, only required if deleting in non-default workspace
delectObject: function (rallyConf, objectType, objectID, workspaceID){
  this.workspaceID = (typeof workspaceID !== 'undefined') ?  workspaceID : '';
  request = rally({
    user:       rallyConf.RALLY_USERNAME,
    pass:       rallyConf.RALLY_PASSWORD,
    apiKey:     rallyConf.RALLY_API_KEY,
  ...
  request.query({
    ...
    query:    queryUtils.where('FormattedID', '=', objectID),
        ...
        request.del({
          ...
          scope: {
              workspace: '/workspace/'+workspaceID  //optional, only required if deleting in non-default workspace
          },
          requestOptions: {} //optional additional options to pass through to request
        }, function(error, result) {
            if(error) {
